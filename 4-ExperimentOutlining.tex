...
Behaviors exhibited during dormancy epochs are not necessarily accompanied with postural changes and displacement of the animal's body.
For instance, pumping-like movement of the proboscis or switch-like movement of haltere tends to happen independent of rest of the body.
...
Such micro-activities potentially occur independently from rest of the body.
...
Classification is done by constructing following frame sets:
\begin{align}
...
As micro-activity bouts are quantified by multiple feature vectors, we determine separate threshold values for each $\V{u}_i$, using the same approach with macro-activity detection, utilizing mixture of Gaussian distributions.
After determining thresholds $c_i$ for each $\V{u}_i$, we construct following frame sets:
\begin{align}
...
\end{align}
For a frame to be outlined as micro-activity, it is sufficient if at least one feature is above corresponding threshold.
This is due to the fact that, micro-activities is manifested by changes of only a small subset of body-parts locations.
