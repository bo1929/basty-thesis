...
As mentioned in Section~\ref{section:challanges}, the two-dimensional nature of the video recordings introduces several significant challenges,  one of which is the occluded body parts. Occlusion describes various cases where the tracked parts are eclipsed by other parts in the view area,
...
Understanding the output of a complex biological system, in our case  the behavior, can only be achieved by studying multiple time-scales concurrently.
