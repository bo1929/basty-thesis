...
Matrices $\mathbf{X} \in \mathbb{R}^{T \times N}$ and $\mathbf{Y} \in \mathbb{R}^{T \times N}$ denotes a multivariate time series for $x$ and $y$ cartesian components of two-dimensional video recordings that are collected for $N$ tracked body parts of the fly on $T$ consecutive time stamps by a pose estimation model.
This multivariate time series matrices $\mathbf{X}$ and $\mathbf{Y}$ are the raw input data that goes into the first step of the feature extraction.
...
\begin{equation}
	\mathbf{G} = \rbrkt{\dot{x}^i_t}_{1 \leq t \leq T, i \in \mathcal{C}^{\prime}} \mid \rbrkt{\dot{y}^i_t}_{1 \leq t \leq T, i \in \mathcal{C}^{\prime}} \mid \rbrkt{d^{i,j}_t}_{1 \leq t \leq T, \cbrkt{i,j} \in \mathcal{D}^{\prime}} \mid \rbrkt{\dot{w}_t^{i,j,k}}_{1 \leq t \leq T, \cbrkt{i,j,k} \in \mathcal{A}^{\prime}} .
\end{equation}

The resulting two feature matrices are $\mathbf{S} \in \mathbb{R}^{T \times \rbrkt{2\card{\mathcal{C}}+\card{\mathcal{D}}+\card{\mathcal{A}}}}$ and $\mathbf{G} \in \mathbb{R}^{T \times \rbrkt{2\card{\mathcal{C}^{\prime}}+\card{\mathcal{D}^{\prime}}+\card{\mathcal{A}^{\prime}}}}$.
...
Let $\tau$ be the window size parameter, i.e., the timescale of interest, then the moving mean of the corresponding gradient feature $\mathbf{g}_{i}$ is given by the function $\mu_\tau$;
\begin{equation}
...
\begin{equation}
	\sigma_\tau(g_{i,t}) = \rbrkt{\sfrac{1}{\min\{t+\tau,T\}-\max\{t-\tau,1\}+1} \sum_{t^{\prime}=\max\{t-\tau,1\}}^{\min\{t+\tau,T\}} \rbrkt{\mu_{\tau}(g_{i,t}) - g_{i, t^{\prime}}}^2}^{\sfrac{1}{2}}
\end{equation}
...
\begin{equation}
	W_{f,t^{\prime}}\sqbrkt{s\rbrkt{t}} = \frac{1}{\sqrt{a\rbrkt{f}}} \int_{-\infty}^{\infty} s\rbrkt{t} \mathit{\Psi}^{\ast}\rbrkt{\frac{t - t^{\prime}}{a(f)}} \dd{t},
\end{equation}
...
\begin{equation}
	a\rbrkt{f} = \frac{w_0 + \sqrt{2+w_0^2}}{4 \pi f},.
\end{equation}

For the discrete sequence of snapshot feature $\mathbf{s}_i$ with sampling period $\Delta t$, $W_{f,t^\prime}[s(t)]$ translates into;
\begin{equation}
	\label{eq:wavelet-practical}
	W_f\rbrkt{\mathbf{s}_i, t^\prime} = \frac{1}{\sqrt{a(f)}} \sum_{t=1}^{T} {\Delta t} s_{i,t} \mathit{\Psi}^{\ast}\rbrkt{\frac{t-t^{\prime}}{a{f}}},
\end{equation}
...
\begin{equation}
	W^0_f\rbrkt{\mathbf{s}_i, t^\prime} = \frac{1}{C\rbrkt{f}} \abs{W_f\rbrkt{\mathbf{s}_i, t^\prime}}.
\end{equation}
...
\begin{equation}
	W^{0}_f\rbrkt{\mathbf{s}_i, t^\prime} = \frac{W_f\rbrkt{\mathbf{s}_i, t^\prime}^2}{a\rbrkt{f}}
\end{equation}
...
\end{equation}
for $i=1,2,\dots,N_f$, and their corresponding wavelet scales are computed via $a\rbrkt{f_i}$.
...
\begin{align}
	\mathbf{W}        & = \sqbrkt{W^0_{\sfrac{1}{f}}\rbrkt{\mathbf{s}_i, t}}_{1 \leq i \leq N_S, \,1 \leq t \leq T, \, \sfrac{1}{f} \in \mathcal{T}_S}, \\
	\mathbf{M}_\mu    & = \sqbrkt{\mu_\tau\rbrkt{g_{i,t}}}_{1 \leq i \leq  N_G,  \, 1\leq t \leq T, \, \tau \in \mathcal{T}_G},                         \\
	\mathbf{M}_\sigma & = \sqbrkt{\sigma_\tau\rbrkt{g_{i,t}}}_{1 \leq i \leq N_G, \, 1\leq t \leq T, \, \tau \in \mathcal{T}_G},
\end{align}
where $\mathbf{S} \in \mathbb{R}^{T \times N_S}$ and $\mathbf{G} \in \mathbb{R}^{T \times N_G}$.

The resulting extended feature tensors of dynamic postural representations are $\mathbf{W} \in \mathbb{R}^{T \times \card{\mathcal{T}_s} \times N_S}$, $\mathbf{M}_\mu \in \mathbb{R}^{T \times \card{\mathcal{T}_v} \times N_G}$ and $\mathbf{M}_\sigma \in \mathbb{R}^{T \times \card{\mathcal{T}_v} \times N_G}$.

\section{Computation of Behavioral Representations}
After applying wavelet transformation or computing moving statistics to extend extracted spatio-temporal features to dynamic postural features, a couple of additional operations are required to generate desired behavioral embeddings.

\subsection{Flattening Dynamic Postural Feature Tensors}
As constructed in Section~\ref{section:dynamic-postural-features}, dynamic postural feature tensors are $\mathbf{W} \in \mathbb{R}^{T \times \card{\mathcal{T}_S} \times N_S}$, $\mathbf{M}_\mu \in \mathbb{R}^{T \times \card{\mathcal{T}_G} \times N_G}$, and  $\mathbf{M}_\sigma \in \mathbb{R}^{T \times \card{\mathcal{T}_G} \times N_G}$.
In order to apply manifold learning-based dimensionality reduction algorithms or traditional machine learning algorithms such as decision trees, one needs to flatten the last two dimensions of these feature tensors.
As a result, feature matrices are $\mathbf{W}^\ast \in \mathbb{R}^{T \times \rbrkt{ N_S\card{\mathcal{T}_S}}}, \mathbf{M}_\mu^\ast \in \mathbb{R}^{T \times \rbrkt{ N_G\card{\mathcal{T}_G}}}$ and $\mathbf{M}_\sigma^\ast \in \mathbb{R}^{T \times \rbrkt{ N_G\card{\mathcal{T}_G}}}$ are obtained.
...
\end{equation}
Similarly, $\textnormal{L}_1$ normalized versions of $\mathbf{M}^\ast_\mu$ and $\mathbf{M}^\ast_\sigma$, namely $\mathbf{M}^\prime_\mu$ and $\mathbf{M}^\prime\sigma$ are obtained.
