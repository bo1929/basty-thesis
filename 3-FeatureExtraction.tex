...
% The values of both $\tau_{\textnormal{median}}$ and $\tau_{\textnormal{boxcar}}$ is typically between $100$ milliseconds to $200$ milliseconds, which corresponds to $\app 3$ frames to $\app 6$ frames in our $30$ FPS setup.
...
\begin{equation}
	\V{\hat{w}}_i = \rbr{\frac{w^\ast_{t,i}}{\sum_{j=1}^{N_S \card{\mathcal{T}_s}}w^\ast_{t, j}}}_{1 \leq t \leq T} \textnormal{ and } \V{\hat{W}}   = \sqbr{\rbr{\V{\hat{w}}_i}_{1 \leq i \leq N_S\card{\mathcal{T}_S}}}.
\end{equation}
