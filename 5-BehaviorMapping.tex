...
The contribution of votes of $\exptAnn_l$ to the committee formed for $\exptUnann_k$ is $\symtt{\symbf{vote}}_f^{k,l} = \qmatrix{\symtt{vote}_{f,1}^{k,l}, \cdots, \symtt{vote}_{f,K}^{k,l}}$, and is given by
\begin{equation}
	\symtt{vote}_{f,i}^{k,l} = \rbr{\log_2(K) - \entropy{\V{\hat{b}}_f^{k,l}}} \hat{b}_{f,i}^{k,l} \qbor \rbr{ 1 - \max_{1 \leq j \leq K} \hat{b}_{f,j}^{k,l}} \hat{b}_{f,i}^{k,l} \qbor \hat{b}_{f,i}^{k,l}.
\end{equation}
...
\begin{equation}
	\hat{y}^k_f = \argmax_{1 \leq i \leq K} \Set{\argmax_{1 \leq j \leq K} { \symtt{vote}_{f,j}^{k,l}}}{j=i, \, 1 \leq l \leq \Ann{R}}.
\end{equation}
...
\begin{equation}
	\hat{y}^k_f = \argmax_{1 \leq i \leq K} \sum_{l=1}^{\Ann{R}} \symtt{vote}_{f,i}^{k,l}.
\end{equation}
