...
Consider two experiments, an annotated one $\exptAnn$, and an unannotated one $\exptUnann$, and their semi-supervised pair embeddings, respectively $\Ann{\V{E}}=\qmatrix{\Ann{\V{e}}_1, \cdots \Ann{\V{e}_{\Ann{F}}}}$, $\Unann{\V{E}}=\qmatrix{\Unann{\V{e}}_1, \cdots \Unann{\V{e}}_{\Unann{F}}}$, where $\Ann{F}$ and $\Unann{F}$ are the total numbers of frames estimated as dormant and active.

\begin{equation}
	\V{b}_f = \qmatrix{b_{f,1}, \cdots, b_{f,K}}.
\end{equation}

\begin{equation}
	b_{f,i} = \begin{dcases}
		\sum_{f^\prime \in \NN{k}{i}{f}}\frac{1}{\dist{\V{e}_f}{\V{e}_f^\prime}^p + \epsilon} & \textnormal{if } \card{\NN{k}{i}{f}} \neq 0, \\
...
\begin{equation}
	N^{\plus}_i = \card*{\Set{f}{y^{\plus}_f=i}}
\end{equation}
