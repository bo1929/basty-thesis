...
The contribution of votes of $\exptAnn_l$ to the committee formed for $\exptUnann_k$ is $\symsf{\symbf{vote}}_f^{k,l} = \qmatrix{\vote_{f,1}^{k,l}, \cdots, \vote_{f,K}^{k,l}}$, and is given by
\begin{equation}
...
\end{equation}
If $\max_i \hat{b}_{f,i}$ is close to $\sfrac{1}{K}$, which means that the computed vector weights the behaviors uniformly, then the factors $\rbr{\log_2(K) - \entropy{\V{\hat{b}}_f^{k,l}}}$ or $\rbr{ 1 - \max_{1 \leq j \leq K} \tilde{b}_{f,j}^{k,l}}$ may be used to decrease the ``importance'' of the vote.
...
The predicted behavioral category at frame $k$ of $\exptUnann_k$, denoted by $\tilde{y}^k_f$, is given by combined votes of the committee.
...
\begin{equation}
	\tilde{y}^k_f = \argmax_{1 \leq i \leq K} \Set{\argmax_{1 \leq j \leq K} { \vote_{f,j}^{k,l}}}{j=i, \, 1 \leq l \leq \Ann{R}}.
\end{equation}

\subsubsection{Soft Voting}
In contrast to majority voting (hard voting), the soft voting scheme assigns the behavioral category as the $\argmax$ of the sum of each annotated experiment's vote vector, and $\tilde{y}^k_f$ is given by
\begin{equation}
	\tilde{y}^k_f = \argmax_{1 \leq i \leq K} \sum_{l=1}^{\Ann{R}} \vote_{f,i}^{k,l}.
\end{equation}
...
Before applying post-processing procedures, the frames in the sets $\MacroActivity$ and $\Quiescent$ should be recovered, as we only predicted the annotations of the frames in the set $\MicroActivity$.
...
Let $\V{\hat{y}^k}=\rbr{\hat{y}^k_1, \cdots, \hat{y}^k_T}$ be the $\exptUnann_k$'s vector of predicted annotations for entire experiment, defined as follows:
\begin{equation}
	\hat{y}^k_t =
	\begin{cases}
		0             & \textnormal{if } t \in \Quiescent_k,                              \\
		\tilde{y}^k_f & \textnormal{if } t \in \MicroActivity_k \textnormal{ and } t=t_f, \\
...
\subsubsection{Pruning Interrupting Bouts}
\begin{equation}
	\hat{y}^k_t = \argmax_y \card{\Set{t^\prime}{\hat{y}^k_{t^\prime}=y, \ \max\cbr{1, t-\tau} \leq t^\prime \leq \min\cbr{t+\tau, T}}}
\end{equation}

\subsubsection{Elimination of Overly Short Bouts}
\begin{equation}
	\hat{y}^k_t=
	\begin{cases}
		0           & \mathsf{bout}^0_t - \mathsf{bout}^1_t < \delta, \\
		\hat{y}^k_t & \textnormal{otherwise},
	\end{cases}
\end{equation}

\subsubsection{Elimination of Overly Long Bouts}
\begin{equation}
	\hat{y}^k_t=
	\begin{cases}
		0           & \mathsf{bout}^0_t - \mathsf{bout}^1_t > \delta, \\
