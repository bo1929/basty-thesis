...
\section{Applying \texttt{basty} to Annotate Experiments}\label{section:employing-proposed-pipeline}
...
We report result and evaluate varying performance for each split.
...
In addition to the hard threshold of confidence score, we apply an adaptive filtering based on the moving median.
In the adaptive filtering, the moving median window size $\tau$ is determined to be $15$ frames ($0.5$ seconds), and threshold $\delta$ is set to $15$ microns.
...
Here, the aim is to reduce the tracking noise and smooth the signal, but without smoothing behaviors exhibited by rapid and short-duration movements.
...
\section{Performance Evaluation}\label{section:performance-evaluation}
Computed behavioral score vectors are used for prediction and also utilized as confidence scores.
