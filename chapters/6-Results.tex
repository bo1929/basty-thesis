...
\section{Applying basty to Annotate Activities }\label{section:employing-proposed-pipeline}

We have 7 wild-type sleep and 4 sleep-deprived fly recordings. We apply the pipeline using a leave-one-out procedure. The annotations of 10 experiments are used for training  and 1 test experiment by leaving one out for each split.
We evaluate  evaluate performance for each split.

\subsection{Preprocessing}
\subsubsection{Identifying Occluded Boyd Parts}
We first extract features. In this step,  raw tracking data obtained by running DeepLabCut is used to generate meaningful behavioral representations.
The features are generated using coordinates of the following body parts: proboscis, haltere, thorax, head (left and right), and three midlegs (left and right).

In the preprocessing step, we first identify the occluded body parts. We use oriented pose values for body parts with left and right counterparts, as described in Section~\ref{section:oriented-pose-values}.  We set the low confidence score threshold to $0.075$.
In addition to the hard threshold applied on the confidence score, we also apply a filtering based on the median window size. The moving median window size $\tau$ is set to $15$ frames ($0.5$ seconds), and threshold for $\tau$ is set to $15$ microns.
If the position of a body part exceeds the median of the window centered around it by threshold tau $\tau$, or if the confidence score is lower than the threshold, then it is deemed that there exists an occluded body part at the corresponding time point, as described in Section~\ref{section:detecting-occlusions}.
Finally, we use linear interpolation to the imputate of such frames with occluded body parts.

\subsection{Noise removal and smoothing}
At the last preprocessing step, we aim to reduce the tracking noise and smooth the signal. We apply a median filter with a window size of $6$ frames ($0.2$ seconds), and then a boxcar filter with a window size of size $6$ frames ($0.2$ seconds). 
...
\subsection{Generation of snapchat features}
In the next step, we computed snapshot features and gradient features as described in Section~\ref{section:spatiotemporal-fetaures}. Details of the snapshot and gradient features are provided in Table~\ref{table:spatiotemporal-features}.
To generate postural dynamics from snapshot features, we applied wavelet transformation at $20$ different frequency channels dyadically spaced between $1$ Hz and $20$ Hz (see Equation~\ref{equation:dyadically-spaced-spectrum} for determining spectrum frequencies). Different timescales are normalized as given in Equation~\ref{equation:wavelet-normalization}. % \citep{liu_rectification_2007}
...
After computing behavioral representations and constructing $\V{\hat{W}}$ for each experiment, we use these for detection of flys' activities during sleep.
...
We evaluate both unsupervised and supervised approaches using recall, and accuracy values ( Figure~\ref{figure:detection-performance}).
...
\begin{figure}[htb!]
	...
	\caption[Performance summary of micro-activity detection.]{Performance summary of micro-activity detection.
		...
		a) Supervised and b) unsupervised detection results. \label{figure:detection-performance}}
\end{figure}
...
The resulting score vectors are used for prediction and also utilized as confidence scores.
...
Characterization of underlying changes in behavioral exhibitions is essential for understanding sleep, and therefore, we also examine behavioral visits that occurred during flies' sleep.
