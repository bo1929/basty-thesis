...
\section{Applying \texttt{basty} to Annotate Activities }\label{section:employing-proposed-pipeline}
We have 7 wild-type sleep and 4 sleep-deprived fly recordings. Our pipeline is evaluated by using a leave-one-out procedure. The annotations of 10 experiments are used for training and 1 test experiment by leaving one out for each split.
We evaluate varying performance for each split.

\subsection{Preprocessing}
\subsubsection{Identifying Occluded Body Parts}
...
Finally, we use linear interpolation to impute of such frames with occluded body parts.

\subsubsection{Noise removal and smoothing}
...
\subsubsection{Generation of snapchat features}
...
\subsection{Results}
\subsubsection{Activity Mapping Performance}
...
\subsubsection{Behavior mapping performance}
...
\subsection{Behaviour Mapping Performance}
\begin{figure}[htb!]
	\centering
